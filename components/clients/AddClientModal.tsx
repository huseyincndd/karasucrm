...
const AddClientModal: React.FC<AddClientModalProps> = ({ isOpen, onClose, onAdd, onUpdate, clientToEdit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      // Custom Quota (Ensure number or null)
      reelsQuota: selectedPackage === 'custom' ? Number(customQuotas.reels) || 0 : null,
      postsQuota: selectedPackage === 'custom' ? Number(customQuotas.posts) || 0 : null,
      storiesQuota: selectedPackage === 'custom' ? Number(customQuotas.stories) || 0 : null,
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {(['star', 'gold', 'premium', 'custom'] as PackageType[]).map(pkg => (
                    <button
                      key={pkg}
                      type="button"
                ...
                <p className="text-sm font-medium text-indigo-700 mt-2">
                  {selectedPackage === 'star' && 'Aylık: 1 Yapay Zeka Video, 5 Grafik Tasarım (3 Post, 2 Story)'}
                  {selectedPackage === 'gold' && 'Aylık: 4 Yapay Zeka Video, 10 Grafik Tasarım (5 Post, 5 Story)'}
                  {selectedPackage === 'premium' && 'Aylık: 6 Reels (Tanıtım dahil), 15 Grafik Tasarım (7 Post, 8 Story)'}
